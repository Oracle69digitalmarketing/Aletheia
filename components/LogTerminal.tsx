...
const mockLogsTemplates = [
  ...
  { level: 'INFO', source: 'PLANNER', message: 'Decomposing milestones based on DeepSeek and Groq reasoning engines.' },
  ...
  { level: 'TRACE', source: 'OPIK', message: 'Pushing intermediate reasoning span to Opik dashboard.' },
...
const LogTerminal: React.FC<LogTerminalProps> = ({ goal }) => {
  ...
  useEffect(() => {
    ...
    const mockLogs = mockLogsTemplates.map(log => ({
      ...log,
      message: log.message.replace('{goal}', goal || 'General Goal').replace('{session}', sessionId)
    }));
