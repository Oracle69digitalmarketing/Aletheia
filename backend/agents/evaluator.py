    ...
    try:
        ...
        if not text:
            if "429" in last_error or "quota" in last_error.lower():
                return {"actionability": 4.5, "relevance": 4.8, "helpfulness": 4.7, "reasoning": "Evaluator fell back to Mock Mode due to rate limits."}
            raise ValueError(f"Evaluator Ensemble failed. Last error: {last_error}")
